...
const MEME_TOKENS = [
  { id: 'moon-worm', symbol: 'WORM', name: 'Moon Worm', theme: ['#d7ff64', '#7df1c7'] },
  { id: 'ghost-peel', symbol: 'PEEL', name: 'Ghost Peel', theme: ['#ffcf7d', '#ff8f52'] },
  { id: 'void-pup', symbol: 'PUP', name: 'Void Pup', theme: ['#85e5ff', '#7df1c7'] },
  { id: 'shadow-frog', symbol: 'FROG', name: 'Shadow Frog', theme: ['#b8ff8a', '#59d68a'] },
  { id: 'neon-rug', symbol: 'RUG', name: 'Neon Rug', theme: ['#ff9bd1', '#ff8f52'] },
  { id: 'quant-cat', symbol: 'CAT', name: 'Quant Cat', theme: ['#d7ff64', '#ffd36e'] }
];
...
const DISPLAY_TOKENS = [...TOKENS, ...MEME_TOKENS];
const TOKENS_BY_ID = Object.fromEntries(DISPLAY_TOKENS.map((token) => [token.id, token]));
...
const dom = {
  ...
  openSimulatorButton: document.getElementById('openSimulatorButton')
};
...
function getSimulatorAccount() {
  ...
  return state.sim?.accounts?.[current.id] || null;
}
...
function renderTokenVisual(token, className) {
  if (MEMES_BY_ID[token.id]) {
    const theme = MEMES_BY_ID[token.id].theme;
    return `<div class="${className.replace('logo', 'badge')}" style="background: linear-gradient(145deg, ${theme[0]}, ${theme[1]});">${escapeHtml(token.symbol)}</div>`;
  }
...
function getFiatBalance(currency) {
  const simAccount = getSimulatorAccount();
  if (simAccount) {
    return simAccount.balances?.[`fiat-${currency}`] || 0;
  }
...
function getAssetUsdPrice(assetId) {
  ...
  if (MEMES_BY_ID[assetId]) {
    return state.sim?.market?.[assetId]?.priceUsd || 0;
  }
...
function getPortfolioRows() {
  const simAccount = getSimulatorAccount();
  const holdings = simAccount
    ? Object.entries(simAccount.balances || {})
      .filter(([assetId, amount]) => !assetId.startsWith('fiat-') && amount > 0)
      .map(([tokenId, amount]) => ({ tokenId, amount }))
    : getCurrentAccount().holdings;

  return holdings
    .map((holding) => {
      const token = TOKENS_BY_ID[holding.tokenId];
      if (!token) return null;
      const price = MEMES_BY_ID[holding.tokenId]
        ? { usd: state.sim?.market?.[holding.tokenId]?.priceUsd || 0, change24h: null }
        : state.prices[holding.tokenId];
...
function render() {
  ...
  dom.assetsSubtitle.textContent = `${summary.rows.length} holding${summary.rows.length === 1 ? '' : 's'}`;
...
function openSimulator() {
  const url = globalThis.chrome?.runtime?.getURL
    ? chrome.runtime.getURL('simulator/index.html')
...
dom.openSimulatorButton.addEventListener('click', openSimulator);
